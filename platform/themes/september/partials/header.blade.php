        ...
        <nav class="navigation--mobile">
            ...
            <div class="navigation__right">
                <div class="header__actions">
                    <a class="search-btn" href="#" title="{{ __('Search') }}"><i
                            class="feather icon icon-search"></i></a>
                    @if (is_plugin_active('ecommerce'))
                        @if (0)
                            <a href="{{ route('customer.login') }}" title="{{ __('Login') }}"><i
                                    class="feather icon icon-user"></i></a>
                        @endif
                        @if (EcommerceHelper::isWishlistEnabled())
                            <a class="btn-shopping-cart btn-wishlist" href="{{ route('public.wishlist') }}"
                                title="{{ __('Wishlist') }}"><i class="feather icon icon-heart"></i>
                        ...
                        @if (EcommerceHelper::isCartEnabled())
                            <a class="btn-shopping-cart panel-trigger" href="#panel-cart" title="{{ __('Cart') }}">
                                <i
                                    class="feather icon icon-shopping-cart"></i><span>{{ Cart::instance('cart')->count() }}</span>
                            </a>
                        @endif
                    @endif
                        ...
                        <div class="d-flex align-items-center justify-content-center justify-content-md-start">
                            <ul class="contact-detail text-center text-lg-left">
                                @if ($hotline = theme_option('hotline'))
                                    <li><i class="feather icon icon-phone"></i>&nbsp;<span>{{ $hotline }}</span>
                                    </li>
                                @endif
                                @if ($email = theme_option('email'))
                                    <li><i class="feather icon icon-mail"></i>&nbsp;<a
                            ...
                            @if (is_plugin_active('ecommerce'))
                                ...
                                @if (count($currencies) > 1)
                                    <div class="language-wrapper choose-currency mr-3">
                                        <div class="dropdown">
                                            <button class="btn btn-secondary dropdown-toggle btn-select-language"
                                        ...
                                        @if (!auth('customer')->check())
                                            <li>
                                                <a href="{{ route('customer.login') }}"><i
                                                        class="feather icon icon-user"></i>&nbsp;<span>{{ __('Login') }}</span></a>
                                            </li>
                                        @else
                                            <li><a href="{{ route('customer.overview') }}"><i
                                                        class="feather icon icon-user"></i>&nbsp;<span>{{ auth('customer')->user()->name }}</span></a>
                                            </li>
                                            <li><a href="{{ route('customer.logout') }}"><i
                                                        class="feather icon icon-lock"></i>&nbsp;<span>{{ __('Logout') }}</span></a>
                                            </li>
                                        @endif
                                    @endif

                                    @if (EcommerceHelper::isCompareEnabled())
                                        <li><a href="{{ route('public.compare') }}"><i
                                                    class="feather icon icon-shuffle"></i>&nbsp;<span>{{ __('Compare') }}
                                                    <span
                                                        class="compare-count">(<span>{{ Cart::instance('compare')->count() }}</span>)</span></span></a>
                                        </li>
                                    @endif
                                </ul>
                            @endif
            ...
            <div class="container">
                ...
                <div class="navigation__right">
                    <div class="header__actions">
                        ...
                        @if (is_plugin_active('ecommerce'))
                            @if (EcommerceHelper::isWishlistEnabled())
                                <a class="btn-shopping-cart btn-wishlist" href="{{ route('public.wishlist') }}"><i
                                        class="feather icon icon-heart"></i>
                                    @if (auth('customer')->check())
                                        <span>{{ auth('customer')->user()->wishlist()->count() }}</span>
                                    @else
                                        <span>{{ Cart::instance('wishlist')->count() }}</span>
                            ...
                            @if (EcommerceHelper::isCartEnabled())
                                <a class="btn-shopping-cart panel-trigger" href="#panel-cart"
                                    title="{{ __('Cart') }}"><i
                                        class="feather icon icon-shopping-cart"></i><span>{{ Cart::instance('cart')->count() }}</span></a>
                            @endif
                        @endif
                    </div>
                </div>
