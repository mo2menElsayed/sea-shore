    ...
    public function buttons(): array
    {
        ...
        if (EcommerceHelper::isEnabledSupportDigitalProducts() && !EcommerceHelper::isDisabledPhysicalProduct()) {
            $buttons['create'] = [
                ...
                'class' => 'btn-primary',
                'buttons' => [
                    // Physical product button (always shown if has basic create permission)
                    [
                        ...
                        'enabled' => $this->hasPermission('products.create') &&
                            $this->hasPermission('products.create.physical')
                    ],
                    // Digital product button (requires additional permission)
                    [
                        'className' => 'action-item',
                        ...
                        'enabled' => $this->hasPermission('products.create') &&
                            $this->hasPermission('products.create.digital')
                    ],
                ],
            ];

            // Remove create dropdown if only one option is available
            if (!$this->hasPermission('products.create.digital')) {
                $buttons = $this->addCreateButton(route('products.create'), 'products.create');
            }
            if (!$this->hasPermission('products.create.physical')) {
                $buttons = $this->addCreateButton(route('products.create', ['product_type' => 'digital']), 'products.create');
            }
