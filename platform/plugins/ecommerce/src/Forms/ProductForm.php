...
class ProductForm extends FormAbstract
{
    public function setup(): void
    {
        ...
        $this
            ...
            ->add('content', EditorField::class, ContentFieldOption::make()->when(
                !auth()->user()->roles->contains('slug', 'seashore'),
                fn($option) => $option->allowedShortcodes()
            ))
            ...
            ->when(\Auth::user()->hasPermission('products.featured'), function (ProductForm $form) use ($tags): void {
                $form
                    ->add(
                        'is_featured',
                        OnOffField::class,
            ...
            ->when(EcommerceHelper::isCartEnabled(), function (ProductForm $form): void {
                $form->when(\Auth::user()->hasPermission('products.min.order'), function (ProductForm $subForm): void {
                    $subForm
                        ->add(
                ...
                $form->when(\Auth::user()->hasPermission('products.max.order'), function (ProductForm $subForm): void {
                    $subForm
                        ->add(
            ...
            ->when(\Auth::user()->hasPermission('products.tags'), function (ProductForm $form) use ($tags): void {
                $form
        ...
        if ($productId && is_in_admin(true) && \Auth::user()->hasPermission('products.duplicate')) {
