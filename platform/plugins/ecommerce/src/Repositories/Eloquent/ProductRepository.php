...
class ProductRepository extends RepositoriesAbstract implements ProductInterface
{
    ...
    public function filterProducts(array $filters, array $params = [])
    {
        ...
        if ($attributes) {
            ...
            // Convert attribute IDs to integers if it's a flat list
            $attributes = $attributesIsList
                ? array_map(fn($id) => (int) $id, $attributes)
                : $attributes;
            if (!$attributesIsList) {
                // Handle associative array (attribute sets)
                foreach ($attributes as $attributeSet => $attributeIds) {
                    // dd(!is_array($attributeIds) || empty(array_filter($attributeIds)));
                    if (!is_array($attributeIds) || empty(array_filter($attributeIds))) {
                        continue;
                    }

                    $this->model->whereExists(function (Builder $query) use ($attributeSet, $attributeIds) {
                        $query->select(DB::raw(1))
                            ...
                            ->when(!EcommerceHelper::showOutOfStockProducts(), function (Builder $query) {
                                $query->join('ec_products as product_children', 'product_children.id', 'ec_product_variations.product_id')
                                    ->where(function (Builder $query) {
                                        $query->where(function ($query) {
                                            $query->where('product_children.with_storehouse_management', 0)
                                                ->whereNot('product_children.stock_status', StockStatusEnum::OUT_OF_STOCK);
                                        })->orWhere(function ($query) {
                                            $query->where('product_children.with_storehouse_management', 1)
                ...
                dd($attributes);
                // Handle simple attribute ID list
                $this->model->whereHas('variations', function ($query) use ($attributes) {
