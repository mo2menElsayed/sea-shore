class Ecommerce {
    ...
    constructor() {
        $(document)
            ...
            .on('submit', 'form.bb-product-form-filter', (e) => {
                // @code
                console.log('product filter 2');
                // e@code
                e.preventDefault()

                const form = $(e.currentTarget)
                // console.log('Raw:', form.serializeArray())
                const formData = this.#transformFormDataFilter(form.serializeArray())
                // // console.log('Raw2:', this.#transformFormData(form.serializeArray()))
                ...
                formData.forEach(item => {
                    ...
                    if (!paramsByName[name].includes(item.value)) {
                        paramsByName[name].push(item.value)
                    }
                })
                ...
                Object.entries(paramsByName).forEach(([name, values]) => {
                    values.forEach(value => {
                        // Automatically handles attributes[color][], etc.
                ...
                $('.panel--sidebar .panel__close').trigger('click')
    ...
    #transformFormDataFilter = (formData) => {
        let data = []
        ...
        formData.forEach((item) => {
            ...
            //     return
            // }
            // Normalize attributes[color] → attributes[color][]
            if (name.startsWith('attributes[') && !name.endsWith('[]')) {
                return
            }
            if (name.startsWith('brands') && !name.endsWith('[]')) {
                return
            }
            data.push({ name, value: item.value })
        })
